...
function dataLocalStr(d) {
    return formatadorDataInput(d);
}

// Parseia string "YYYY-MM-DD" como horário local (evita shift de fuso UTC)
function parseDateLocal(str) {
    if (!str) return null;
    // Adiciona T00:00:00 sem Z para forçar interpretação como hora local
    return new Date(str.trim() + 'T00:00:00');
}
...
function montarBaseDeDados(dadosF3 = [], dadosFFQP = []) {
    ...
    dadosF3.forEach(row => {
        if (row.data) {
            row.loja = "F3";
            row.parsedDate = parseDateLocal(row.data);
...
window.aplicarFiltros = function () {
    ...
    const diasF3 = new Set(dadosF3.map(d => dataLocalStr(d.parsedDate))).size || 1;
    const diasFFQP = new Set(dadosFFQP.map(d => dataLocalStr(d.parsedDate))).size || 1;
...
function atualizarGraficos(dadosF3, dadosFFQP, filtroLoja) {
    ...
    dadosF3.forEach(d => {
        let k = dataLocalStr(d.parsedDate);
